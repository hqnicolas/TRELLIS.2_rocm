...
// ROCm: replaced int3_decomposer with uint64 encoding helpers
__global__ void encode_int3_kernel(const int3* faces, uint64_t* keys, int N) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= N) return;
    keys[tid] = ((uint64_t)(uint32_t)faces[tid].x << 42) |
                ((uint64_t)(uint32_t)faces[tid].y << 21) |
                ((uint64_t)(uint32_t)faces[tid].z);
}
__global__ void decode_int3_kernel(const uint64_t* keys, int3* faces, int N) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= N) return;
    faces[tid].x = (int)((keys[tid] >> 42) & 0x1FFFFF);
    faces[tid].y = (int)((keys[tid] >> 21) & 0x1FFFFF);
    faces[tid].z = (int)(keys[tid] & 0x1FFFFF);
}
...
void CuMesh::remove_duplicate_faces() {
    ...
    // ROCm: encode int3 -> uint64, sort, decode back
    uint64_t *cu_face_keys, *cu_face_keys_output;
    CUDA_CHECK(cudaMalloc(&cu_face_keys, F * sizeof(uint64_t)));
    CUDA_CHECK(cudaMalloc(&cu_face_keys_output, F * sizeof(uint64_t)));
    encode_int3_kernel<<<(F+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(cu_sorted_faces, cu_face_keys, F);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
        nullptr, temp_storage_bytes,
        cu_face_keys, cu_face_keys_output,
    ...
    decode_int3_kernel<<<(F+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(cu_face_keys_output, cu_sorted_faces_output, F);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaFree(cu_face_keys));
    CUDA_CHECK(cudaFree(cu_face_keys_output));
