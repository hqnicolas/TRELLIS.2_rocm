...
void CuMesh::init(const torch::Tensor& vertices, const torch::Tensor& faces) {
    ...
    // FIX: Use cudaMemcpy instead of cudaMemcpy2D to avoid gfx11-class ROCm memcpy bugs
    // This is a simple 1D contiguous copy, not a true 2D copy with different strides
    // cudaMemcpy2D has known memory corruption issues on ROCm 7.x with this hardware
    CUDA_CHECK(cudaMemcpy(
        this->vertices.ptr,
