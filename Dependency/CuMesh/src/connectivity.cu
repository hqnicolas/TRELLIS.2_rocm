...
void CuMesh::get_edges() {
    ...
    //validate_vertices("before_resize_edges");

    this->edges.resize(F * 3);

    // Checkpoint 2: After edges resize
    //validate_vertices("after_resize_edges");
    ...
    // Checkpoint 3: After kernel
    //validate_vertices("after_kernel");

    // sort edges + unique edges
    this->temp_storage.resize(F * 3 * sizeof(uint64_t));

    // Checkpoint 4: After temp_storage resize
    //validate_vertices("after_resize_temp_storage");
/*
#ifdef USE_ROCM
    // ROCm gfx11-class workaround: hipcub DeviceRadixSort and DeviceRunLengthEncode
    // corrupt adjacent GPU memory for large N (>~512K). Do both on CPU instead.
    {
        size_t N = F * 3;
        std::vector<uint64_t> host_edges(N);

        // Checkpoint 5: Before D2H copy
        validate_vertices("before_memcpy_d2h");

        CUDA_CHECK(cudaMemcpy(host_edges.data(), this->edges.ptr,
                              N * sizeof(uint64_t), cudaMemcpyDeviceToHost));

        // Checkpoint 6: After D2H copy
        validate_vertices("after_memcpy_d2h");

        std::sort(host_edges.begin(), host_edges.end());

        // Run-length encode on CPU: produce unique edges + counts
        std::vector<uint64_t> unique_edges;
        std::vector<int>      edge_counts;
        unique_edges.reserve(N);
        edge_counts.reserve(N);
        size_t i = 0;
        while (i < N) {
            size_t j = i + 1;
            while (j < N && host_edges[j] == host_edges[i]) ++j;
            unique_edges.push_back(host_edges[i]);
            edge_counts.push_back(static_cast<int>(j - i));
            i = j;
        }
        int num_unique = static_cast<int>(unique_edges.size());

        // Checkpoint 7: Before edges resize (shrink)
        validate_vertices("before_resize_edges_shrink");

        this->edges.resize(num_unique);

        // Checkpoint 8: After edges resize (shrink)
        validate_vertices("after_resize_edges_shrink");

        // Checkpoint 9: Before H2D copy
        validate_vertices("before_memcpy_h2d");

        CUDA_CHECK(cudaMemcpy(this->edges.ptr, unique_edges.data(),
                              num_unique * sizeof(uint64_t), cudaMemcpyHostToDevice));

        // Checkpoint 10: After H2D copy
        validate_vertices("after_memcpy_h2d");

        // Checkpoint 11: Before edge2face_cnt resize
        validate_vertices("before_resize_edge2face_cnt");

        this->edge2face_cnt.resize(num_unique);

        // Checkpoint 12: After edge2face_cnt resize
        validate_vertices("after_resize_edge2face_cnt");

        CUDA_CHECK(cudaMemcpy(this->edge2face_cnt.ptr, edge_counts.data(),
                              num_unique * sizeof(int), cudaMemcpyHostToDevice));

        // Checkpoint 13: Final state
        validate_vertices("final");

        this->edges.size      = num_unique;
        this->edge2face_cnt.size = num_unique;
    }
    // Skip the GPU RLE path entirely on ROCm.
    int* num_edges = nullptr;  // unused in ROCm path; set size above already
    (void)num_edges;
#else*/
    size_t temp_storage_bytes = 0;
 ...
 * This is a debugging function that logs vertex statistics to a file.
 */
void CuMesh::validate_vertices(const char* checkpoint_name) {
    static float3* h_vertices = nullptr;
    static size_t h_vertices_size = 0;

    // Allocate host buffer once
    if (h_vertices == nullptr || h_vertices_size < this->vertices.size) {
        if (h_vertices != nullptr) delete[] h_vertices;
        h_vertices = new float3[this->vertices.size];
        h_vertices_size = this->vertices.size;
    }

    // Copy vertices to host
    CUDA_CHECK(cudaMemcpy(h_vertices, this->vertices.ptr,
                          this->vertices.size * sizeof(float3),
                          cudaMemcpyDeviceToHost));

    // Find min/max
    float3 min_val = {FLT_MAX, FLT_MAX, FLT_MAX};
    float3 max_val = {-FLT_MAX, -FLT_MAX, -FLT_MAX};
    bool has_nan = false;

    for (size_t i = 0; i < this->vertices.size; i++) {
        min_val.x = fmin(min_val.x, h_vertices[i].x);
        min_val.y = fmin(min_val.y, h_vertices[i].y);
        min_val.z = fmin(min_val.z, h_vertices[i].z);
        max_val.x = fmax(max_val.x, h_vertices[i].x);
        max_val.y = fmax(max_val.y, h_vertices[i].y);
        max_val.z = fmax(max_val.z, h_vertices[i].z);

        if (isnan(h_vertices[i].x) || isnan(h_vertices[i].y) || isnan(h_vertices[i].z)) {
            has_nan = true;
        }
    }

    // Log to file
    FILE* f = fopen("cumesh_vertex_validation.log", "a");
    fprintf(f, "[VALIDATION] %s\n", checkpoint_name);
    fprintf(f, "  vertices: %zu\n", this->vertices.size);
    fprintf(f, "  min: [%.8f %.8f %.8f]\n", min_val.x, min_val.y, min_val.z);
    fprintf(f, "  max: [%.8f %.8f %.8f]\n", max_val.x, max_val.y, max_val.z);
    fprintf(f, "  has_nan: %s\n", has_nan ? "YES" : "NO");
    fprintf(f, "  vertices.ptr: %p\n", (void*)this->vertices.ptr);
    fprintf(f, "  edges.ptr: %p\n", (void*)this->edges.ptr);
    fprintf(f, "  temp_storage.ptr: %p\n", (void*)this->temp_storage.ptr);
    fprintf(f, "\n");
    fclose(f);

    // Check for corruption
    static float3 initial_max = {-FLT_MAX, -FLT_MAX, -FLT_MAX};
    static bool first_run = true;
    if (first_run) {
        initial_max = max_val;
        first_run = false;
    } else {
        if (fabs(max_val.x - initial_max.x) > 0.001f ||
            fabs(max_val.y - initial_max.y) > 0.001f ||
            fabs(max_val.z - initial_max.z) > 0.001f) {
            fprintf(stderr, "[CORRUPTION DETECTED] at %s\n", checkpoint_name);
            fprintf(stderr, "  Initial max: [%.8f %.8f %.8f]\n",
                    initial_max.x, initial_max.y, initial_max.z);
            fprintf(stderr, "  Current max: [%.8f %.8f %.8f]\n",
                    max_val.x, max_val.y, max_val.z);
        }
    }
}
