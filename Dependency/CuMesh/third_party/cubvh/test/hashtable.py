import torch
from cubvh.api import HashTable


def _cpu_int(arr: torch.Tensor) -> torch.Tensor:
    return arr.to(dtype=torch.int32, device='cpu', non_blocking=True).contiguous()
...
def test_3d_basic():
    ...
    coords = _cpu_int(coords)

    ht = HashTable(num_dims=3)
    ht.build(coords)
...
def test_large_random(num_dims: int = 3, num_keys: int = 200000, num_queries: int = 100000):
    ...
    ht = HashTable(num_dims=num_dims)
    ht.build(coords)
...
if __name__ == "__main__":
    print("Running CPU HashTable tests...")
    test_3d_basic()
    test_4d_basic()
    test_large_random()
    test_edge_values()
    print("All CPU HashTable tests passed.")
