...
// Lane mask functions - wave32 mode: lane is in [0,31], all masks fit in U32.
// Use ull arithmetic to avoid UB on lane=31 (1u<<32 overflows), then cast to U32.
__device__ __inline__ U32 getLaneMaskLt(void) { unsigned int lane = __lane_id(); return (U32)((lane > 0) ? ((1ull << lane) - 1ull) : 0ull); }
__device__ __inline__ U32 getLaneMaskLe(void) { unsigned int lane = __lane_id(); return (U32)((1ull << (lane + 1)) - 1ull); }
...
__device__ __inline__ bool singleLane(void) { return (__ballot(true) & getLaneMaskLt()) == 0u; }
