    ...
    if (!binEmpty)
      do {
        ...
        while (triQueueWritePos - triQueueReadPos <= CR_COARSE_WARPS * 32) {
          ...
          U32 hasStreamMask = __ballot_sync(~0ull, hasStream);
          if (hasStream) {
        ...
        if (__any_sync(~0u, triIdx != -1)) {
          ...
          if (__all_sync(~0u, sizex <= 2 && sizey <= 2)) {
            ...
            if (__any_sync(~0u, warea * 4 <= area * 8)) {
              ...
              bool act = (triIdx != -1);
              U32 actMask = __ballot_sync(~0ull, act);
              if (act) {
        ...
        for (int tileInBin_base = 0; tileInBin_base < CR_BIN_SQR;
          ...
          if (act) {
            ...
            if (!__any_sync(actMask, tileEmits >= 2)) {
              U32 m = getLaneMaskLe();
              *v = (__popc(__ballot_sync((unsigned long long)actMask, tileEmits & 1) & m)
                    << emitShift) |
                   __popc(__ballot_sync((unsigned long long)actMask, tileAllocs & 1) & m);
            }
        ...
        U32 scan8Mask = __ballot_sync(~0ull, scan8);
        if (scan8) {
